...
STDMETHODIMP_(void*) CDX7SubPic::GetObject() const
{
    return (void*)(IDirectDrawSurface7*)m_pSurface;
}

STDMETHODIMP CDX7SubPic::GetDesc(SubPicDesc& spd) const
{
    DDSURFACEDESC2 ddsd;
    INITDDSTRUCT(ddsd);
...
// pSubPic must be a pointer to CDX7SubPic in fact because implementation of this function use an
// explicit cast the type of the return value of GetObject to IDirectDrawSurface7*
...
STDMETHODIMP CDX7SubPic::AlphaBlt(const RECT* pSrc, const RECT* pDst, SubPicDesc* pTarget)
{
    ASSERT(pTarget == NULL);
