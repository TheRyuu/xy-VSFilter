                                              ...
                                              CompositeDrawItemList* compDrawItemList /*output*/)
{
    ...
    if (s->m_hard_position_level > POS_LVL_NONE) {
        ...
        switch (s->m_scrAlignment % 3) {
            case 1:
                margin.x = video_org.x - margin_rect.left;
                break; //move to left
            case 2:
                margin.x = video_org.x;
                break; //do not move so that it aligns with the middle of the video
            case 3:
                margin.x = video_org.x + margin_rect.right;
                break; //move to right
        }
        switch ((s->m_scrAlignment - 1) / 3) {
            case 0:
                margin.y = video_org.y - margin_rect.top;
                break; //move to top
            case 1:
                margin.y = video_org.y;
                break; //do not move so that it aligns with the middle of the video
            case 2:
                margin.y = video_org.y + margin_rect.bottom;
                break;//move to bottom
        }
